...
## movie guess section ##
def movieguess(guessfield, hiddenmovie, game, man):
...
def hint():
	hint = "The movie was released in " + title_year + " and directed by " + director_name + ".\n" + "Starring " + actor_1_name + ", " + actor_2_name + ", and " + actor_3_name + "."
...
def startgame():
	...
	bguessletter = Button(game, text="Guess Letter", width=15, command=lambda:letterguess(guessfield, hiddenmovie, game, gamelabel1, man))
	bguessletter.pack()

	bguessmovie = Button(game, text="Guess Word [ONE CHANCE]", width=25, command=lambda:movieguess(guessfield, hiddenmovie, game, man))
	bguessmovie.pack()

	bhint = Button(game, text="Hint", width=15, command=hint)
	bhint.pack()
...
def win(game, movie):
	tkinter.messagebox.showinfo("Winnerx2-Chicken-Dinner", "You WIN! The movie was " + movie + "!")
	game.withdraw()

def lose(game, movie):
	tkinter.messagebox.showinfo("Loser-Shmooser", "You LOSE! The movie was " + movie + "!")
	game.withdraw()
...
movie_metadata_connection = sqlite3.connect('database/movie_metadata.sqlite')
